...
[[ -z "$NASM_FILE" ]] && rm "$NASM_FILE"
gcc -m32 -c -o "$O_FILE" "$C_FILE"
c2nasm/objconv -fnasm "$O_FILE" "$NASM_FILE"
rm "$O_FILE"
...
nasm -O1 -f elf -g -F dwarf -o "$NASM_O_FILE" "$NASM_FILE"
ld -melf_i386 -o main "$NASM_O_FILE"
#nasm -:qf elf32 -o "$NASM_O_FILE" "$NASM_FILE"
#gcc -m32 -lGL -lm -o "$EXEC_FILE"  "$NASM_O_FILE"
echo 'Successfully compiled '"$NASM_FILE"
cat "$NASM_FILE"
